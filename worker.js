  ...
  async fetch(request) {
    ...
    if (u.pathname === "/") {
      return new Response(JSON.stringify({
        ok:true,
        service:"Hong Kong Traffic Control Center Proxy",
        version:"ULTIMATE-4"
      }), {headers:{...cors,"Content-Type":"application/json;charset=utf-8"}});
    }

    let target="", type="application/json;charset=utf-8";
    ...
    if (u.pathname === "/mtr") {
      ...
      if(!target || !allowed.some(x=>target.startsWith(x))){
        return new Response(JSON.stringify({ok:false,error:"Invalid MTR URL"}),{status:400,headers:{...cors,"Content-Type":type}});
      }
      ...
      return new Response(JSON.stringify({ok:false,error:"Not found"}),{status:404,headers:{...cors,"Content-Type":type}});
    }

    try{
      const r=await fetch(target,{headers:{"User-Agent":"HKTransportControlCenter/ULTIMATE-4"}});

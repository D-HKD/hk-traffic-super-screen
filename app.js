...
async function loadWeather(){try{const j=await(await get('/weather')).json();const r=j.weather||j,temps=r.temperature?.data||[],hums=r.humidity?.data||[],station=window.gpsStation?.name||'屯門';const t=temps.find(x=>String(x.place).includes(station))||temps[0],h=hums.find(x=>String(x.place).includes(station))||hums[0],temp=t?.value,hum=h?.value,icon=wxIcon(r.icon);$('weather-icon').textContent=icon;$('weather-icon-large').textContent=icon;$('header-temp').textContent=(temp??'--')+'°C';$('big-temp').textContent=(temp??'--')+'°C';$('header-weather').textContent=station+'附近';$('big-weather').textContent=station+'附近天氣';$('weather-station').textContent=station;$('header-humidity').textContent=(hum??'--')+'%';$('big-humidity').textContent=(hum??'--')+'%';window.lastWeather={station,temp,hum}}catch(e){console.warn(e)}}
...
async function loadTraffic(){try{const items=parseTraffic(await(await get('/trafficnews')).text());const focus=items.filter(x=>/屯門|元朗|天水圍|朗屏|青山|屯門公路|元朗公路|大欖/i.test(x.title+' '+x.detail+' '+x.loc));const list=(focus.length?focus:items).slice(0,4);$('alert-list').innerHTML=list.length?list.map(x=>`<article class="alert-item"><div><b>${esc(x.title)}</b><span class="alert-time">${esc(x.time)}</span></div><p>${esc(x.loc?x.loc+'：':'')}${esc(x.detail).slice(0,190)}</p></article>`).join(''):'<span class="muted">目前沒有交通消息</span>';$('alert-count').textContent=list.length+' 則';$('road-status').textContent=focus.length?'留意消息':'大致暢順';$('road-tuenmun-text').textContent=focus.length?'留意消息':'大致暢順';$('traffic-update').textContent=new Date().toLocaleTimeString('zh-HK',{hour12:false});$('alert-time').textContent=new Date().toLocaleTimeString('zh-HK',{hour12:false});window.lastTraffic=focus.length?focus[0].detail:'目前未發現屯門及元朗特別交通消息'}catch(e){console.warn(e);$('road-status').textContent='連線重試'}}
...
function speak(){if(!('speechSynthesis'in window))return;const w=window.lastWeather||{station:'香港',temp:'--',hum:'--'},t=window.lastTraffic||'目前未發現特別交通消息';speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(`香港交通控制中心即時報告。${w.station}附近目前氣溫 ${w.temp} 度，濕度 ${w.hum}%。交通方面，${t}。屯馬綫及輕鐵正在更新實時班次。`);u.lang='zh-HK';u.rate=.86;u.pitch=$('voice-gender').value==='male'?.88:.98;const v=voiceFor($('voice-gender').value);if(v){u.voice=v;u.lang=v.lang}speechSynthesis.speak(u)}
$('voice-now').onclick=speak;$('voice-start').onclick=()=>{speak();clearInterval(timer);timer=setInterval(speak,+$('voice-interval').value*60000)};$('voice-stop').onclick=()=>{clearInterval(timer);timer=null;speechSynthesis?.cancel()};$('night-btn').onclick=()=>document.body.classList.toggle('night');$('full-btn').onclick=()=>document.fullscreenElement?document.exitFullscreen?.():document.documentElement.requestFullscreen?.();
